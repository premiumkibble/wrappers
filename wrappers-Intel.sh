...
docker pull haocsmt/sosi:wrappers && docker run -d  --rm --hostname `whoami`-wrappers --name wrappers -w /root/myhome -p 127.0.0.1:3389:3389 -e ip=$ip -e DISPLAY=host.docker.internal:0 -v ~/DockerHome/wrappers:/root -v ~/:/root/myhome -v ~/Dockerhome/wrappers/bin:/usr/local/sbin -i -t haocsmt/sosi:wrappers /bin/bash -c 'startup && /bin/bash || /bin/bash' 
exit
